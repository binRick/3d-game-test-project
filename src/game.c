...
static void UpdEnemies(float dt) {
    for (int i=0;i<g_ec;i++) {
        ...
        // Arch-vile (type 14) resurrection AI — overrides the usual
        // chase-the-player loop. Each frame we hunt for the nearest
        // dying corpse within 10 m (excluding chef boss / cyber demon /
        // other arch-viles, which would be game-breaking to revive).
        // If found, walk toward it; once within 1.5 m, revive: clear
        // dying, restore 60% HP, kick into CHASE state, splash blood.
        // After a successful revive the vile cools down for 2 s before
        // looking again. If no corpse is reachable, fall through to the
        // standard player-chase / melee AI below.
        if (e->type == 14) {
            if (e->cd > 0.f) e->cd -= dt;
            if (e->cd <= 0.f) {
                int corpse = -1;
                float bestD2 = 10.f * 10.f;
                for (int j = 0; j < g_ec; j++) {
                    Enemy *o = &g_e[j];
                    if (j == i || !o->active || !o->dying) continue;
                    // Skip bosses (3, 9), pain elementals (12 — flying corpse
                    // weirdness), and other arch-viles. Everything else is fair game.
                    if (o->type == 3 || o->type == 9 || o->type == 12 || o->type == 14) continue;
                    float odx = o->pos.x - e->pos.x;
                    float odz = o->pos.z - e->pos.z;
                    float od2 = odx*odx + odz*odz;
                    if (od2 < bestD2) { bestD2 = od2; corpse = j; }
                }
                if (corpse >= 0) {
                    Enemy *o = &g_e[corpse];
                    float odx = o->pos.x - e->pos.x;
                    float odz = o->pos.z - e->pos.z;
                    float od  = sqrtf(odx*odx + odz*odz);
                    if (od < 1.5f) {
                        // Revive! 60% HP, fresh chase target.
                        o->dying  = false;
                        o->hp     = o->maxHp * 0.6f;
                        o->deathT = 0.f;
                        o->flashT = 0.f;
                        o->state  = ES_CHASE;
                        Blood(o->pos, 30);
                        e->cd = 2.f;  // cooldown before next revive search
                    } else if (od > 0.01f) {
                        EnemyMove(e, (odx/od) * e->speed * dt,
                                     (odz/od) * e->speed * dt);
                        e->legT += dt * e->speed * 2.8f;
                    }
                    e->flashT = (e->flashT > 0.f) ? e->flashT - dt : 0.f;
                    continue;  // skip standard player-chase logic
                }
            }
            // No corpse to revive — fall through, vile chases player.
        }
    ...
    for (int k = 0; k < n; k++) {
        ...
        if (e->type >= 7 && e->type <= 17) {
            ...
            if (pe->ok) {
                ...
                // plant on the floor.
                float feetBias = (e->type == 16) ? 1.00f : 0.0f;  // spider mastermind
